...
/// `Y² = X(X-1)(X-λ)`,
///
/// then the standard complete-elliptic-integral periods for the invariant
/// differential `dX / Y` are
///
/// `2 K(λ)` and `2 i K(1-λ)`.
///
...
///
/// So this routine recovers the concrete period basis
///
/// `ω₁ = 2 scale K(λ)`,
/// `ω₂ = 2 scale i K(1-λ)`.
///
...
/// standard complete-elliptic-integral periods
///
/// `2 K(λ)` and `2i K(1 -λ )`
///
/// for the differential `dX / Y`. Multiplying by `differential_scale`
/// therefore yields the candidate periods for the original curve:
///
/// `ω₁ = 2 * differential_scale * K(λ)`,
/// `ω₂ = 2i * differential_scale * K(1 - λ)`.
///
...
fn build_period_basis_from_legendre_data(
    ...
    let omega1 = Complex64::new(2.0, 0.0) * differential_scale * *integral_report.k_lambda.value();
    let omega2 =
        Complex64::new(0.0, 2.0) * differential_scale * *integral_report.k_complementary.value();

    if !complex_is_finite(&omega1) || !complex_is_finite(&omega2) {
