use crate::elliptic_curves::{
    ...
    traits::{
        ...
        GroupCurveModel, HasJInvariant, LiftXCoordinate, LiftedPoints,
    },
};
...
fn enumerable_curve_model_lists_the_finite_identity_exactly_once() {
    ...
    assert_eq!(
        all_points.iter().filter(|point| curve.is_identity(point)).count(),
        1
    );
...
fn point_order_matches_the_montgomery_companion_on_the_common_open_subset() {
    ...
    assert_eq!(curve.point_order(&point), montgomery.point_order(&montgomery_point));
}
