...
///
/// The stored `root` selects one of those two conjugate choices by recording
/// the local congruence ω ≡ r (mod ℓ)
///
/// Here `ω` denotes the standard generator of the quadratic order, so that
/// `O_f = ℤ[ω]`. Equivalently, this is the split prime ideal generated by `ℓ`
/// and `ω - r`, often written 𝔭_r = (ℓ, ω - r).
///
...
impl SplitPrimeIdeal {
    ...
    ///
    /// The chosen root records the congruence `ω ≡ root (mod ℓ)` for the
    /// standard generator `ω` with `O_f = ℤ[ω]`.
    ///
