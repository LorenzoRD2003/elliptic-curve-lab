...
use crate::elliptic_curves::{AffineCurveModel, HasseInterval, ShortWeierstrassCurve};
...
fn build_center_heavy_benchmark_corpus(
    ...
    for a in 1..=search_budget {
        ...
        let Some(annihilating_multiple) = find_annihilating_multiple_in_interval_bsgs_with_config(
            &curve,
            &point,
            interval.clone(),
            left_to_right_config,
        )
        .expect("benchmark corpus search should succeed") else {
            continue;
        };
...
fn benchmark_bsgs_corpus_iteration(
    ...
    for instance in corpus {
        let annihilating_multiple = find_annihilating_multiple_in_interval_bsgs_with_config(
            &instance.curve,
            &instance.point,
            interval.clone(),
            config,
