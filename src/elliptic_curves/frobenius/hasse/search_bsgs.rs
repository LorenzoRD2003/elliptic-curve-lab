...
impl HasseBsgsConfig {
    ...
    fn traversal(self) -> HasseBsgsTraversal {
        self.traversal
    }

    fn uses_fast_negation(self) -> bool {
        self.use_fast_negation
    }

    fn known_parity(self) -> HasseBsgsParity {
        self.known_parity
    }
}
...
///
/// If `config.known_parity` is `Even` or `Odd`, the search is first restricted
/// to that congruence class inside `H(q)` by rewriting the candidates as
/// `M = M_0 + 2k`. Equivalently, the baby steps advance by `[2]P`, which is
/// the same parity optimization described in the notes.
///
/// Complexity: Let `c = |H(q) ∩ ℤ|`. The baseline implementation chooses
...
///
/// With known parity, the candidate set size drops to roughly `c / 2`, so the
/// baby-step and giant-step counts each improve by a factor of about `√2`.
...
///
/// Current status:
/// - `LeftToRight + fast negation` is the tuned default
/// - `Even` / `Odd` parity genuinely reduce the candidate set to one residue
///   class modulo `2`
/// - `MiddleOut` now uses two center-out giant-step frontiers rather than one
///   giant-step state that repeatedly repositions across alternating blocks
///
/// Interpretation:
/// - use `LeftToRight` when you want the strongest fixed-instance baseline
/// - use `MiddleOut` when you explicitly want to test the semicircular-style
///   heuristic from the notes
    ...
    C::Point: Clone + Eq + Hash,
{
    ...
    match config.known_parity() {
        HasseBsgsParity::Unknown => {
            ...
            let candidate_offset = mul_scalar_biguint(curve, point, &BigUint::from(first))?;
...
fn find_progression_solution_bsgs<C: GroupCurveModel + ?Sized>(
    curve: &C,
    step_point: &C::Point,
    target: &C::Point,
    ...
    C::Point: Clone + Eq + Hash,
{
    ...
    let giant_stride = mul_scalar_biguint(curve, step_point, &BigUint::from(giant_stride_width))?;
...
struct BsgsSearchContext<'a, C: GroupCurveModel + ?Sized>
where
    C::Point: Clone + Eq + Hash,
{
...
fn find_progression_solution_bsgs_left_to_right<C: GroupCurveModel + ?Sized>(
    curve: &C,
    ...
    C::Point: Clone + Eq + Hash,
{
    ...
    let initial_multiple = mul_scalar_biguint(
        curve,
        context.step_point,
        &BigUint::from(initial_candidate_index),
    )?;
...
fn find_progression_solution_bsgs_middle_out<C: GroupCurveModel + ?Sized>(
    curve: &C,
    context: &BsgsSearchContext<'_, C>,
) -> Result<Option<u128>, CurveError>
where
...
/// `-giant`.
fn candidate_for_block_match<C: GroupCurveModel + ?Sized>(
    curve: &C,
    context: &BsgsSearchContext<'_, C>,
    giant: &C::Point,
    block_index: u128,
