...
use crate::elliptic_curves::endomorphisms::{
    binary_quadratic_forms::{
        BinaryQuadraticForm, QuadraticClassGroup, QuadraticClassGroupCayleyTable,
...
fn describe_generated_subgroup_by_set(
    subgroup: &QuadraticClassGroupGeneratedSubgroupBySet,
) -> String {
    let mut lines = vec![
        "Quadratic class-group generated subgroup by set".to_string(),
        "------------------------------------------------".to_string(),
        format!("discriminant D: {}", subgroup.discriminant().value()),
        format!(
            "generators: {}",
        ...
        format!(
            "observed structure: {}",
            generated_set_structure_label(subgroup)
        ),
        "elements:".to_string(),
    ];

    for (index, element) in subgroup.elements().iter().enumerate() {
        lines.push(format!("  s{index} = {}", element.format_compact()));
    }

    lines.push(
        "interpretation: this is an algebraic subgroup generated by several form classes; a geometric action would need one certified local crater orientation per generator."
            .to_string(),
    );
    lines.push(
        "construction cost: up to m·h(D) class compositions, O(m · h(D) · C) total".to_string(),
    );
    lines.join("\n")
}

fn generated_set_structure_label(subgroup: &QuadraticClassGroupGeneratedSubgroupBySet) -> String {
    if subgroup.order() == 1 {
        return "trivial".to_string();
    }

    if subgroup.order() == 4 && generated_set_is_klein_four(subgroup) {
        return "ℤ/2ℤ × ℤ/2ℤ".to_string();
    }

    "not classified by this educational formatter".to_string()
}

fn generated_set_is_klein_four(subgroup: &QuadraticClassGroupGeneratedSubgroupBySet) -> bool {
    let Ok(class_group) = QuadraticClassGroup::new(subgroup.discriminant().clone()) else {
        return false;
    };
    let Some(identity) = subgroup.elements().first() else {
        return false;
    };

    for element in subgroup.elements().iter().skip(1) {
        let Ok(square) = class_group.compose(element, element) else {
            return false;
        };
        if &square != identity {
            return false;
...
impl Visualizable for QuadraticClassGroupGeneratedSubgroupBySet {
    fn format_compact(&self) -> String {
        format!(
            "subgroup generated by {} classes in Cl(D = {}): order {}/{}",
            self.generators().len(),
            self.discriminant().value(),
            self.order(),
            self.class_number()
    ...
    fn describe(&self) -> String {
        describe_generated_subgroup_by_set(self)
    }
}
...
mod tests {
    ...
    #[test]
    fn generated_subgroup_by_set_description_explains_the_klein_case() {
        let class_group = QuadraticClassGroup::new(QuadraticDiscriminant::new(-84))
            .expect("D = -84 should be supported");
        let subgroup = class_group
            .generated_subgroup_by_set(&[
                BinaryQuadraticForm::new(2.into(), 2.into(), 11.into()),
                BinaryQuadraticForm::new(3.into(), 0.into(), 7.into()),
            ])
            .expect("two D = -84 generators should generate a subgroup");

        let text = subgroup.describe();

        assert!(text.contains("Quadratic class-group generated subgroup by set"));
        assert!(text.contains("generators: (2,2,11), (3,0,7)"));
        assert!(text.contains("subgroup order: 4"));
        assert!(text.contains("ambient class number h(D): 4"));
        assert!(text.contains("generates whole class group: yes"));
        assert!(text.contains("observed structure: ℤ/2ℤ × ℤ/2ℤ"));
        assert!(text.contains("one certified local crater orientation per generator"));
        assert_eq!(
            subgroup.format_compact(),
            "subgroup generated by 2 classes in Cl(D = -84): order 4/4"
        );
    }
}
